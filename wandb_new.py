...
class PPO_MODEL:
    ...
    def __init__(self, env: SubprocVecEnv, device_num, load_path=None):
        self.name = env_name
    ...
    def train(self):
        ...
        for _ in tqdmLoader:
            ...
            for i in range(0, self.env_num*self.steps, self.batch_size):
                s_input = s[i:i+self.batch_size, :].cuda(self.device)
                a_input = a[i:i+self.batch_size, :].cuda(self.device)
                v_input = v[i:i+self.batch_size, :].cuda(self.device)
                adv_input = adv[i:i+self.batch_size].cuda(self.device)
                adv_input = (adv_input - adv_mean) / (adv_std + 1e-8)
                logp_input = logp[i:i+self.batch_size].cuda(self.device)
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            for i in tqdmLoader:
                ...
                s_input = torch.stack(s, dim=1).cuda(self.device)
                a_input = torch.stack(a, dim=1).cuda(self.device)
                ...
                for j in range(self.env_num):
                    ...
                    if done[j] == True or i == self.steps-1:
                        ...
                        if done[j] == True:
                            ...
                            if i % 100 == 0:
                                wandb.log({"results/avg_obs_mean": self.obs_mean.abs().mean().item(),
                                           "results/avg_obs_std": self.obs_std.abs().mean().item(),
                                           "results/avg_trajectory_length": full_trajectory_length/full_trajectory_num,
                                           "results/avg_trajectory_reward": full_reward/full_trajectory_num,
                                           "results/avg_step_reward": all_reward/(i+1)/self.env_num}, step=self.now_round * self.env_num * self.steps + i)
    ...
    def gen_video(self, name=None, use_mean_action=True):
        ...
        with torch.no_grad():
            ...
            while done == False:
                ...
                s_tensor = s_tensor.unsqueeze(1).cuda(self.device)
                a_tensor = a_tensor.unsqueeze(1).cuda(self.device)

                action, action_logp, value = self.network(
                    s_tensor.cuda(self.device), a_tensor.cuda(self.device),
                ...
                wandb.log({"results/sum_reward": sum_reward},
                          step=self.now_round*self.env_num*self.steps)
        ...
        env.close()
        print('\n'+'--- finish rendering---')
        return [frame.transpose(2, 0, 1) for frame in video]

    def learn(self):
        dir = './' + self.name+'/'
        video = self.gen_video()
        wandb.log(
            {"results/video": wandb.Video(np.stack(video), fps=100, format="gif")})
...
if __name__ == "__main__":
    ...
    PPO_MODEL(env, ARGS.device).learn()

    # env_name = "h1hand-walk-v0"
    # env = SubprocVecEnv([make_env(i) for i in range(1)])
    # model = PPO_MODEL(env, './our_checkpoints/50.pt')
