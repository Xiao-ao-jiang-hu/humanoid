...
class SACAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch: Batch):
        ...
        def critic_loss_fn(critic_params):
            ...
            if agent.config['backup_entropy']:
                target_q = target_q - \
                    agent.config['discount'] * batch['masks'] * \
                    next_log_probs * agent.temp()
    ...
    def sample_actions(agent,
        ...
        actions = agent.actor(
            observations, temperature=temperature).sample(seed=seed)
...
def create_learner(
    ...
    actor = TrainState.create(actor_def, actor_params,
    ...
    temp = TrainState.create(temp_def, temp_params,
